...
def run_command(command, error_message):
    ...
    try:
        ...
        logging.error(
            f"{error_message}: {e.stderr.decode()} (Exit code: {e.returncode})"
        )
        return False
    return True
...
def load_and_format_linearb_corpus(csv_path):
    ...
    for word in unique_words_series:
        ...
        linear_b_chars = reg.findall(
            r"[\U00010000-\U0001007F\U00010080-\U000100FF]", word
        )
        if linear_b_chars:
...
def load_and_format_brown_corpus():
    ...
    corpus_name = "brown"
    letter_filter = get_letter_filter(
        corpus_name
    )  # Retrieve the appropriate letter filter for Brown corpus
        ...
        for word in sent:
            ...
            if len(cleaned_word) >= 3:
                ...
                if filtered_letters:
                    ...
                    all_chars.extend(
                        filtered_letters
                    )  # Collect letters for unique_letters count
...
def load_and_format_europarl_corpus(language):
    ...
    letter_filter = get_letter_filter(
        corpus_name
    )  # Retrieve the appropriate letter filter for the language
    ...
    try:
        ...
        logging.error(
            f"Language '{language}' not found in europarl_raw corpus."
        )
        return "", 0, 0
...
def calculate_entropy_kenlm(model, text):
    ...
    if isinstance(text, list):
        text = "\n".join(
            text
        )  # Join lines with newline to preserve word boundaries

    log_prob = model.score(text, bos=False, eos=False) / math.log(2)
    num_grams = max(
        len(text.split()) - Q_GRAMS + 1, 1
    )  # Correct formula for n-grams
...
def process_corpus(corpus_name, q_gram):
    ...
    if corpus_name == "linear_b":
        formatted_text, vocab_count, unique_letters = (
            load_and_format_linearb_corpus(CORPUS_PATH)
        )
        lowercase = False
    elif corpus_name == "brown":
        formatted_text, vocab_count, unique_letters = (
            load_and_format_brown_corpus()
        )
        lowercase = True
    else:
        formatted_text, vocab_count, unique_letters = (
            load_and_format_europarl_corpus(corpus_name)
        )
        lowercase = True

    if len(formatted_text.split()) < q_gram:
        logging.error(
            f"Insufficient data in {corpus_name.title()} corpus to build a language model."
        )
        return

    model_path = build_kenlm_model(
        formatted_text, MODEL_DIR, q_gram, corpus_name
    )
    ...
    if model_path:
        ...
        H_max = (
            math.log2(len(unique_letters_set))
            if len(unique_letters_set) > 0
            else 0
        )
        H1 = calculate_unigram_entropy(
            formatted_text, letter_filter, corpus_name
        )
        H2 = calculate_H2(formatted_text, letter_filter, corpus_name)
        H3_kenlm = calculate_entropy_kenlm(
            model, lines
        )  # Correct H3 calculation
        ...
        logging.info(
            f"Grapheme Inventory: {len(unique_letters_set)}"
        ...
        logging.info(
            f"Third-order Entropy (H3) of {Q_GRAMS}-grams: {H3_kenlm:.2f}"
        )
        ...
        try:
            ...
            logging.error(
                f"Failed to delete model files for {corpus_name}, error: {e}"
            )
