...
def calculate_unigram_entropy(text, corpus_name):
    ...
    probabilities = (
        np.array(list(unigram_freq.values())) / total_unigrams
        if total_unigrams > 0
        else np.array([])
    )
...
def calculate_H2(text, corpus_name):
    ...
    # Calculate probabilities
    probabilities = (
        np.array([count / total_chars for count in char_freq.values()])
        if total_chars > 0
        else np.array([])
    )
...
def process_single_corpus(corpus_name):
    ...
    text_for_kenlm, vocab_count, grapheme_inventory = load_and_format_corpus(
        corpus_name
    )
    ...
    model_path = build_kenlm_model(
        text_for_kenlm, MODEL_DIR, Q_GRAMS, corpus_name
    )
    if model_path:
...
if __name__ == "__main__":
    corpora = sys.argv[1:] if len(sys.argv) > 1 else ALL_CORPORA
